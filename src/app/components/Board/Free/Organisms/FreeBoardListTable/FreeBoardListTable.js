...
import { useState } from 'react'; // 상태 관리 위해 추가
...
export default function BoardListTable() {
  // 공지 제외한 일반 게시글만 필터링
  const noticePosts = dummyPosts.filter(post => post.isNotice);
  const normalPosts = dummyPosts.filter(post => !post.isNotice);
  const [postsPerPage, setPostsPerPage] = useState(30);


  // 일반 게시글에 number 필드 부여 (최신글이 위에 오게)
  const numberedPosts = normalPosts.map((post, idx) => ({
    ...post,
    number: normalPosts.length - idx, // 최신글 위, 1부터 시작
  }));
  ...
  return (
    <section className={styles.boardSection}>
      ...
      <div className={styles.infoBar}>
        <span>게시물 총 {dummyPosts.length}개</span>
        {/* <span className={styles.count}>30개⌵</span> */}
